...
void Unit::DealDamageMods(Unit* victim, uint32 &damage, uint32* absorb)
{
    ...
    if (victim != this && IsControlledByPlayer() && victim->IsControlledByPlayer())
    {
...
void Unit::DealMeleeDamage(CalcDamageInfo *damageInfo, bool durabilityLoss)
{
    ...
    if ((damageInfo->hitOutCome == MELEE_HIT_CRIT || damageInfo->hitOutCome == MELEE_HIT_CRUSHING || damageInfo->hitOutCome == MELEE_HIT_NORMAL || damageInfo->hitOutCome == MELEE_HIT_GLANCING) &&
        GetTypeId() != TYPEID_PLAYER && !this->ToCreature()->IsControlledByPlayer() && !victim->HasInArc(M_PI, this)
...
void Unit::AttackerStateUpdate (Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    if ((attType == BASE_ATTACK || attType == OFF_ATTACK) && !this->IsWithinLOSInMap(victim))
        return;
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellEntry const *spell)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
        HitChance -= int32(victim->ToPlayer()->GetRatingBonusValue(CR_HIT_TAKEN_SPELL) * 100.0f);
    ...
    switch(dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(dummySpell->Id)
            {
                ...
                case 33757:
                {
                    if (GetTypeId() != TYPEID_PLAYER || !castItem || !castItem->IsEquipped()  || !victim || !victim->isAlive())
                        return false;
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth)
{
    ...
    if (unit->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (gain)
            unit->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HEALING_DONE, gain, 0, victim);
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        victim->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_TOTAL_HEALING_RECEIVED, gain);
        victim->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEALING_RECEIVED, addhealth);
    }
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_PALADIN:
                        ...
                        {
                            // Sacred Shield
                            AuraEffect const* aura = victim->GetAuraEffect(58597, 1);
