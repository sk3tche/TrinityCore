    ...
    if (m_MirrorTimerFlags & (UNDERWATER_INLAVA | UNDERWATER_INSLIME))
    {
        ...
        else
        {
            ...
            if (m_MirrorTimer[FIRE_TIMER] < 0)
            {
                ...
                // Need to skip Slime damage in Undercity and lordaeron arena
                // This is because liquid data isn't extracted properly to maps
...
void Player::RewardReputation(Unit* victim, float rate)
{
    ...
    ReputationOnKillEntry const* Rep = sObjectMgr->GetReputationOnKilEntry(victim->ToCreature()->GetCreatureInfo()->Entry);
...
uint32 Player::GetArenaTeamIdFromDB(uint64 guid, ArenaType type)
{
