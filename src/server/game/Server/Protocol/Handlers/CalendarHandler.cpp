...
{
    // Packet is empty
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: CMSG_CALENDAR_GET_CALENDAR");
    sLog->outString("CMSG_CALENDAR_GET_CALENDAR");

    time_t cur_time = time(NULL);

    // we can't really get the real size of this packet...
    WorldPacket data(SMSG_CALENDAR_SEND_CALENDAR, 4+4*0+4+4*0+4+4);
    sLog->outString("SMSG_CALENDAR_SEND_CALENDAR");

    sLog->outString("AppendInvitesToCalendarPacketForPlayer");
    sCalendarMgr->AppendInvitesToCalendarPacketForPlayer(data, GetPlayer());
    sLog->outString("AppendEventsToCalendarPacketForPlayer");
    sCalendarMgr->AppendEventsToCalendarPacketForPlayer(data, GetPlayer());

    InstanceSave* save = NULL;
    uint32 counter = _player->m_boundInstances[DUNGEON_DIFFICULTY_NORMAL].size() + _player->m_boundInstances[DUNGEON_DIFFICULTY_HEROIC].size();
    sLog->outString("counter = %u", counter);
    ...
    uint32 constTime = 1135753200;
    data << constTime;                             // Wednesday 7AM, raids reset time. (28.12.2005 07:00)
...
{
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: CMSG_CALENDAR_GET_NUM_PENDING");
