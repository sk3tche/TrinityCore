...
uint32 const SPELL_ENTANGLE[]   = {720, 731, 1121};
    ...
    public:
        ...
        struct boss_fankrissAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                _events.ScheduleEvent(EVENT_SPAWN_HATCHLING, 12000);
                _events.ScheduleEvent(EVENT_SPAWN_WORM, urand(30000, 45000));
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SPAWN_HATCHLING:
                            _summonCount = urand(0,2);
                            _events.ScheduleEvent(EVENT_HATCHLING_TIMER, 500);
                            _events.ScheduleEvent(EVENT_SPAWN_HATCHLING, urand(45000, 60000));
                            break;
                        case EVENT_HATCHLING_TIMER:
                            if (_summonCount >= 0)
                                SpawnHatchling(SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true), _summonCount);
                            _summonCount--;
                            _events.ScheduleEvent(EVENT_HATCHLING_TIMER, 1500);
                            break;
