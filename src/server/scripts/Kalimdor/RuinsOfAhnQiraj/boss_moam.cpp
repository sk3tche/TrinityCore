            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    ...
                    case ACTION_STONE_PHASE_START:
                    {
                        ...
                        _events.ScheduleEvent(EVENT_STONE_PHASE_END, 90000);
                        break;
                    }
                    default:
                        break;
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DRAIN_MANA:
                        {
                            ...
                            _events.ScheduleEvent(EVENT_DRAIN_MANA, urand(5000,15000));
                            break;
                        }/*
...
class spell_moam_energize : public SpellScriptLoader
{
    public:
        ...
        class spell_moam_energize_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_moam_energize_AuraScript);

            bool Validate(SpellEntry const* spell)
            {
                if (!sSpellStore.LookupEntry(SPELL_ENERGIZE))
                    return false;
                return true;
            }
            ...
            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_moam_energize_AuraScript::PeriodicTick, EFFECT_1, SPELL_AURA_PERIODIC_ENERGIZE);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_moam_energize_AuraScript();
        }
};*/
