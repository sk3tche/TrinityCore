...
enum ShamanSpells
{
    ...
    SHAMAN_SPELL_GLYPH_OF_STONECLAW_TOTEM   = 63298,
    SHAMAN_SPELL_MANA_TIDE_TOTEM            = 39609,
    SHAMAN_SPELL_FIRE_NOVA_R1               = 1535,
    SHAMAN_SPELL_FIRE_NOVA_TRIGGERED_R1     = 8349,
    SHAMAN_SPELL_STONECLAW_TOTEM_ABSORB     = 55277,
...
// 55328 (including ranks) - Stoneclaw Totem
class spell_sha_stoneclaw_totem_absorb : public SpellScriptLoader
{
    public:
        spell_sha_stoneclaw_totem_absorb() : SpellScriptLoader("spell_sha_stoneclaw_totem_absorb") { }

        class spell_sha_stoneclaw_totem_absorb_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_sha_stoneclaw_totem_absorb_SpellScript);
            bool Validate(SpellEntry const* /*spellEntry*/)
            {
                if (!sSpellStore.LookupEntry(SHAMAN_SPELL_STONECLAW_TOTEM_ABSORB))
                    return false;
                if (!sSpellStore.LookupEntry(SHAMAN_SPELL_GLYPH_OF_STONECLAW_TOTEM))
                    return false;
                return true;
            }
                ...
                if (Player* target = GetHitPlayer())
                {
                    for (uint8 slot = SUMMON_SLOT_TOTEM; slot < MAX_TOTEM_SLOT; ++slot)
                    {
                        if (target->m_SummonSlot[slot])
                            if (Creature* totem = target->GetMap()->GetCreature(target->m_SummonSlot[slot]))
                                if (totem->isTotem())
                                    GetCaster()->CastCustomSpell(totem, SHAMAN_SPELL_STONECLAW_TOTEM_ABSORB, &damage, NULL, NULL, true);
                    }
                    // Glyph of Stoneclaw Totem
                    if (AuraEffect* aur = target->GetAuraEffect(SHAMAN_SPELL_GLYPH_OF_STONECLAW_TOTEM, effIndex))
                    {
                        damage *= aur->GetAmount();
                        GetCaster()->CastCustomSpell(target, SHAMAN_SPELL_STONECLAW_TOTEM_ABSORB, &damage, NULL, NULL, true);
                    }
                }
            }
            ...
            void Register()
            {
                OnEffect += SpellEffectFn(spell_sha_stoneclaw_totem_absorb_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_sha_stoneclaw_totem_absorb_SpellScript();
        }
};
...
void AddSC_shaman_spell_scripts()
{
    ...
    new spell_sha_stoneclaw_totem_absorb();
}
